...
def Q_push(Q,seg): # добавление координат точек в траекторию заметающей прямой (по возрастанию х с приоритетом координат точек начала)
    ...
    for i in range(0,2):
        ...
        while (j < len(Q)) and (seg[i][0] > Q[j][0]):
            j += 1
            min_x_index = j
        if (min_x_index != len(Q)) and (seg[i][0] == Q[min_x_index][0]) and (i == 1): ## только 2 точка, чтобы подвинуть концы отрезков дальше 
...
def L_refresh(L, y_max, y_min, x, A): # обновление статуса заметающей прямой
    line = [[x,y_min],[x,y_max],[x,y_min]]
    len_L = len(L)
    for i in range(0,len_L):
        L[i][2] = cross(line, L[i])
    for i in range (len_L,1,-1):
        ...
        max_y = max(L[j][2][1] for j in range (i-1,-1,-1))
        j = 0
        while (j < i) and (L[j][2][1] != max_y):
            j += 1
...
def L_push(seg, L, A): # вставка нового отрезка в список текущих проверяемых отрезков
    len_L = len(L)
    min_y_index = 0
    i = 0
    while (i < len_L) and (seg[0][1] > L[i][2][1]):
        i += 1
        min_y_index = i
...
def L_pop(seg, L): # удаление отрезка из списока текущих проверяемых отрезков
    ...
    while (i < len_L) and (seg_index == -1):
        if (seg[0] == L[i][0]) and (seg[1] == L[i][1]):
            seg_index = i
        i += 1
    L.pop(seg_index)
    for i in range (0,len_L-1):
        if (not A_belong(A,[[L[i][0],L[i][1]],[seg[0],seg[1]]])
            and L[i][0][0] <= seg[0][0] <= L[i][1][0] and (L[i][0][1] <= seg[0][1] <= L[i][1][1] or L[i][1][1] <= seg[0][1] <= L[i][0][1])
            and area(L[i][0],L[i][1],seg[0]) == 0):
            print([[L[i][0],L[i][1]],[seg[0],seg[1]]])
            A.append([[L[i][0],L[i][1]],[seg[0],seg[1]]])

def belong(L,seg): # проверка принадлежности отрезка seg к ССО
    len_L = len(L)
    i = 0
...
def exist_y(L,p): # ф-я поиска такого отрезка в ССО, чтобы его у-координата начала совпадала с у-координатой точки р, но при этом х-координаты отличались
    if (L[0][2][0] != p[0]):
        i = 0
        while (i < len(L) and (L[i][2][1]) != p[1]):
            i += 1
        if i == len(L):
            return False
        else:
...
def find(P, L, p): # поиск такого отрезка seg из Р, у которого одна из координат == p, в приоритете начала отрезков, которх нет в списке заметающей прямой ССО
    i = 0
    seg = None
    while (i < len(P)):
...
def VPO(P): # поиск всех пересекающихся отрезков на плоскости
    ...
    for i in range (0,len(a)):
        a[i].insert(2,a[i][0])
    y_max = max(max(a[i][0][1],a[i][1][1]) for i in range (0,len(a)))
    y_min = min(min(a[i][0][1],a[i][1][1]) for i in range (0,len(a)))
    LP = []
    SL = []
    C = []
    push_prev = None
    ...
    for i in range (0,len(a)):
        Q_push(LP,a[i])
    print ("\nСПИСОК ТОЧЕК СОБЫТИЙ (точек перемещения прямой SL):")
    ...
    while len(LP) != 0:
        ...
        seg = find(a,SL,p)
        if (p == seg[0]):
            if (len(SL) != 0 and exist_y(SL,p)):     ## обновляем статус, только когда y-координата предыдущей
            ...
            L_pop(seg,SL,C)
            pop_prev = p[0]
    print ("\nСПИСОК ПАР ПЕРЕСЕККАЮЩИХСЯ ОТРЕЗОВ:")
...
def one_side(A,M): # проверка вершины многоугольника - True, если оба ребра, формирующих вершину, находятся по одну сторону от самой вершины
    ...
    if (P[0][1] == line_y):
        ...
        if (P[0][1] > line_y):
            ...
            down = 1
    i = 0
    while (i < len(A)) and (up == 1 and down == None or down == 1 and up == None):
        J = A[i][1]
        if (((J[0] == k) and (J[1][1] > line_y)) or ((J[1] == k) and (J[0][1] > line_y)) ):
            up = i
        elif (((J[0] == k) and (J[1][1] < line_y)) or ((J[1] == k) and (J[0][1] < line_y)) ):
            down = i
        i += 1
    if (up == None):
        A.pop(down)
        ...
        if (i == len_C):
            ...
            for i in range (0,len(K)):
                if (K[i][1][0][1] == K[i][1][1][1]):
                    K.insert(0,K.pop(i))
            print ("\nПЕРЕСЕЧЕНИЯ ГОРИЗОНАЛЬНОЙ ОТРЕЗКА ",line, "С РЁБРАМИ ВНЕШНЕГО МНОГОУГОЛЬНИКА:")
            for i in range(0,len(K)):
                print("CROSS[",i,"] = ",K[i])
            i = 0
            while (i < len(K)):
...
L = [[[2,3],[2.5,3]],  ## треугольник для проверки с I
     [[2.5,3],[2.5,2]],
     [[2.5,2],[2,3]]]


I = [[[1,3],[3,1]], ## на закрепление
     [[3,1],[7,1]],
     [[7,1],[7,3]],
     [[7,3],[5,3]],
     [[5,3],[5,2]],
